                   ...
                   krb5_boolean use_dns, krb5_const_principal *princ_out)
{
    ...
    char *hostname = NULL, *canonhost = NULL, **hrealms = NULL;
    ...
    *princ_out = NULL;
    ...
    /* If there was a trailer, add it to the expanded hostname. */
    free(iter->canonhost);
    if (trailer.length > 0) {
        if (asprintf(&iter->canonhost, "%s%.*s", canonhost,
